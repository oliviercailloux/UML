...
Une association peut indiquer, à un bout, une aggrégation [en fait indiquée par la propriété], soit shared soit composite (cf. https://en.wikipedia.org/wiki/Object_composition#/media/File:Aggregation-Composition3.png[exemple Wikipedia]: `University` est un aggrégat (_composite_) d’éléments `Department`, eux-mêmes aggrégeant (_shared_) des éléments `Professor`). Association qualifiée pour dictionnaires (réduit la multiplicité). 
